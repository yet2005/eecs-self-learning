...
计算训练集的每个元素与测试集的每个元素之间的欧几里得距离的平方。图像应展平并视为向量。
此实现对训练数据和测试数据使用了一组简单的嵌套循环。
输入数据可以有任意数量的维度 - 例如，此函数应该能够计算向量之间的最近邻，在这种情况下，输入将具有形状 (num_{train, test}, D)；
它还应该能够计算图像之间的最近邻，
其中输入将具有形状 (num_{train, test}, C, H, W)。更一般地说
，输入将具有形状 (num_{train, test}, D1, D2, ..., Dn)；在计算距离之前，
您应该将每个形状为 (D1, D2, ..., Dn) 的元素展平为形状为 (D1 * D2 * ... * Dn) 的向量。
输入张量不应被修改。
注意：您的实现不得使用 torch.norm、torch.dist、torch.cdist 或它们的实例方法变体 x.norm/x.dist/x.cdist。
您不得使用 torch.nn 或 torch.nn.functional 中的任何函数。
输入：
x_train：形状为 (num_train, D1, D2, ...) 的 Torch 张量
x_test：形状为 (num_test, D1, D2, ...) 的 Torch 张量
返回：
dists：形状为 (num_train, num_test) 的 Torch 张量，其中 dists [i, j] 是第 i 个训练点与第 j 个测试点之间的欧几里得距离的平方。
它应该与 x_train 具有相同的数据类型。
  """
  ...
  for i in range(num_train):
    for j in range(num_test):
      diff=torch.sum((x_train_flat[i]-x_test_flat[j])**2)
      dists[i,j]=diff
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def compute_distances_one_loop(x_train, x_test):
  """
  计算训练集的每个元素与测试集的每个元素之间的欧几里得距离的平方。
  图像应展平并视为向量。
  此实现仅对训练数据使用单个循环。
  与 compute_distances_two_loops 类似，此函数应能够处理任意数量维度的输入。
  输入不应被修改。
  Computes the squared Euclidean distance between each element of the training
  set and each element of the test set. Images should be flattened and treated
  as vectors.
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_flat=x_train.view(num_train,-1)
  x_test_flat=x_test.view(num_test,-1)
  sum_test=torch.sum(x_test_flat**2,dim=1)
  sum_train=torch.sum(x_train_flat**2,dim=1)
  cross=x_train_flat@x_test_flat.t()
  dists=sum_test[None,:]+sum_train[:,None]-2*cross
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    """
    使用分类器进行预测。输入：
    x_test形状为 (num_test, C, H, W) 的 PyTorch 张量，表示测试样本
    k用于预测的邻居数量
    返回：
    y_test_pred形状为 (num_test,) 的 PyTorch 张量，给出测试样本的预测标签。
    y_test_pred = None
    ###########################################################################
    待办事项：实现此方法。你应该使用上面编写的函数
    来计算距离（使用无循环变体）并预测输出标签。
    ###########################################################################
    用你的代码替换 “pass” 语句
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for fold_idx in range(num_folds):
        ...
        x_train_cv = torch.cat(x_train_folds[:fold_idx] + x_train_folds[fold_idx+1:], dim=0)
        y_train_cv = torch.cat(y_train_folds[:fold_idx] + y_train_folds[fold_idx+1:], dim=0)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, acc_list in k_to_accuracies.items():
          mean_acc = torch.tensor(acc_list).float().mean()  # 计算每个 k 的平均准确率
          if mean_acc > best_acc:
              best_acc = mean_acc
              best_k = k
          elif mean_acc == best_acc:  # 如果平均准确率相同，选最小 k
              best_k = min(best_k, k)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
