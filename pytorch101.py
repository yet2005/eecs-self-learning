...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  assert start <= stop
    # 找到第一个 >= start 的 10 的倍数
  first = ((start + 9) // 10) * 10
  if first > stop:
      return torch.tensor([], dtype=torch.float64)
    # 用步长 10 生成
  return torch.arange(first, stop + 1, 10, dtype=torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y=x.view(3,2,4).permute(1,0,2).reshape(3,8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    for i in range(x.shape[0]):
      result.append(torch.mm(x[i],y[i]))
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  M=x.shape(0)
  mean=torch.sum(x,dim=0)/M
  diff=x-mean
  var=torch.max(torch.abs(diff),dim=0).values
  y=diff/var
